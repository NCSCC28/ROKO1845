...
export const bibleBookNames = bibleBooks.map((b) => b.name);

export const bibleBookIndex = new Map<string, number>(
  bibleBooks.map((book, index) => [book.name, index])
);

export const bibleBookMeta = new Map<string, BibleBook>(
  bibleBooks.map((book) => [book.name, book])
);
